...
```{r}
load(file="~/GitHub/functional_mediation/data_application/data_application_cPain_gypsie/Data_analysis_cPain_20150304-2125.Rdata")
...
```{r all_together, fig.height=3.5,fig.width=8,dpi=200, include=FALSE}
...
c = c + geom_pointrange(data=point_dta, mapping=aes(x=Parameter, y=t0, ymin=BI025, ymax=BI975), width=0.2, size=1, color="blue", fill="white", shape=1) 
# c + geom_point(size = 3) + geom_errorbar(aes(ymax = BI025, ymin = BI975))+guides(colour=FALSE)
c
```


```{r each_function, fig.height=3.2,fig.width=2.7,dpi=100, include=FALSE}
...
```{r sfs_cPain, fig.height=8,fig.width=8,dpi=300, fig.path='C:/Users/Yenny/Dropbox/sharelatex/Functional_Mediation_Analysis/', include=FALSE}
c = ggplot(data=summ_tbl, aes(y = "t0", x = "time",colour = "Parameter"))
c = c +facet_grid(Parameter ~ ., scale="free")

c$labels = list(x = "Time in seconds", y = "Rating", colour = "Simulation")

c = c + geom_path(data = summ_tbl, aes(x = time, y = t0, group = ID, colour = Parameter), size = 0.7, alpha = 0.05) + guides(colour=FALSE)
