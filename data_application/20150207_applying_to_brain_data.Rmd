...
dta3$time <- as.numeric(as.character(dta3$time))

dta3$Condition <- factor(dta3$X, levels=c(1,0), labels = c("High", "Low"))
```

```{r dta, fig.height=4.7,fig.width=4.7,dpi=100, fig.path='C:/Users/Yenny/Documents/GitHub/functional_mediation/data_application/', include=FALSE}
c = ggplot(data=dta3, aes(y = BOLD, x = time, colour=Condition))
c = c +facet_grid(BR ~ Condition , scale="fixed")

c$labels = list(x = "Time in seconds", y = "Intensity", colour = "Condition")
c = c + geom_path(data = dta3, aes(x = time, y = BOLD, group = ID, colour = Condition), size = 0.7, alpha = 0.05) + guides(colour=FALSE) + stat_smooth(size=2,level = 0.95) 
c
```
...
all_tbl$Parameter = mapvalues(all_tbl$Parameter, from = levels(all_tbl$Parameter), to = c("Alpha*Beta", "Alpha*Beta (t=23)", "Alpha*Beta (t=12)", "Alpha*Beta (t=5)",  "Alpha", "Beta (t=23)","Beta (t=12)","Beta (t=5)", "Delta 1","Delta 2", "Gamma"))

mediator_tbl = filter(all_tbl,Parameter %in% c("Alpha","Delta 1"))
mediator_tbl$Parameter = factor(mediator_tbl$Parameter, levels=c("Alpha","Delta 1"))

outcome_tbl = filter(all_tbl,Parameter %in% c("Delta 2", "Gamma","Beta (t=5)", "Beta (t=12)","Beta (t=23)"))
outcome_tbl$Parameter = factor(outcome_tbl$Parameter, levels=c("Delta 2", "Gamma","Beta (t=5)", "Beta (t=12)","Beta (t=23)"))

indirect_tbl = filter(all_tbl,Parameter %in% c("Beta (t=5)","Beta (t=12)","Beta (t=23)","Alpha", "Alpha*Beta (t=5)","Alpha*Beta (t=12)","Alpha*Beta (t=23)", "Alpha*Beta"))
indirect_tbl$Parameter = factor(indirect_tbl$Parameter, levels=c("Beta (t=5)","Beta (t=12)","Beta (t=23)","Alpha", "Alpha*Beta (t=5)","Alpha*Beta (t=12)","Alpha*Beta (t=23)", "Alpha*Beta"))

small_tbl = filter(all_tbl,Parameter %in% c("Alpha", "Beta (t=12)","Beta (t=23)", "Alpha*Beta", "Gamma"))
small_tbl$Parameter = factor(small_tbl$Parameter, levels=c("Alpha", "Beta (t=12)","Beta (t=23)", "Alpha*Beta", "Gamma"))
```
