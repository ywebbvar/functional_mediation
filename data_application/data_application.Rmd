...
big_dta0.m <- melt(big_dta0, id=c("ID", "n_miss", "X", "Y"), value.name="value")
big_dta0.m2 <- ddply(big_dta0.m, .(variable), transform, scale = scale(value))
```

# Unscaled data
```{r}
p <- ggplot(big_dta0.m, aes(variable, ID)) + geom_tile(aes(fill = value), colour = "white") + scale_fill_gradient(low = "white", high = "steelblue")
base_size <- 9
...
# Scaled data
```{r}
p <- ggplot(big_dta0.m2, aes(variable, ID)) + geom_tile(aes(fill = scale), colour = "white") + scale_fill_gradient(low = "white", high = "steelblue")
base_size <- 9
p <- p + theme_grey(base_size = base_size) + labs(x = "", y = "") + scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(expand = c(0, 0)) + theme(legend.position = "none", axis.ticks = element_blank(), axis.text.x = element_text(size = base_size * 0.8, angle = 330, hjust = 0, colour = "grey50"))
```
