...
fbootstrap_ML = function(dta, index){
  ...
  result = sff_Mediation(X,Y,M,mediatorMethod="fosr2s", nbasis=nbasis,norder=norder,lambda=lambda, plot=FALSE, boot=TRUE)
  return(result)
}
```
...
for(i in 1:iters){
  ...
  system.time(
  out <- sff_Mediation(X,Y,t(M),mediatorMethod="fosr2s",penalty_ff=c(3,2), nbasis=nbasis,norder=norder,lambda=lambda, plot=TRUE, boot=TRUE) # Using cubic splines for tensor product in ff(), penalty=c(2,1)
  )
