...
fbootstrap_ML = function(dta, index){
  ...
  result = sff_Mediation(X,Y,M,mediatorMethod="fosr2s",penalty_ff=c(3,3),nbasis=nbasis,norder=norder,lambda=lambda, plot=FALSE, boot=TRUE)
  return(result)
}
```
...
Pall2 = matrix(0, iters, (2*len^2+5*len))
...
for(i in 1:iters){
  ...
  system.time(
  out <- sff_Mediation(X,Y,t(M),mediatorMethod="fosr2s",penalty_ff=c(3,3), nbasis=nbasis,norder=norder,lambda=lambda, plot=TRUE, boot=TRUE) # Using quartic splines with penalty on the third derivative for tensor product in ff(), penalty=c(3,3)
  )
  ...
  # Calculating p-value
  Pall[i,] = 2*apply(cbind(colMeans(stat$t < 2*matrix(stat$t0, ncol=ncol(stat$t), nrow=nrow(stat$t), byrow = TRUE)),colMeans(stat$t > 2*matrix(stat$t0, ncol=ncol(stat$t), nrow=nrow(stat$t), byrow = TRUE))),1, min)
  all[i,] = stat$t0
  }
p_bf  = t(matrix(Pall[i,grep("^bsurface_", names(Pall[i,]))], len, len))
p_abf = t(matrix(Pall[i,grep("absurface_", names(Pall[i,]))], len, len))

colnames(Pall) = colnames(all) = names(stat$t0)

save(all,Pall,Pall2, file=paste0("Simulation_4_",format(Sys.time(), "%Y%m%d-%H%M"),".Rdata"))
```
...
for(i in 1:iters){
  ...
  Y    = t(sapply(1:sub, function(i) create_functional_y(x=X[i], m=M[i,], etai=eta[i,], delta2=delta2, gamma=gamma, beta=matrix(0, ncol=length(beta), nrow=length(beta)))))
...
save(all,Pall,Pall2, file=paste0("Simulation_1_",format(Sys.time(), "%Y%m%d-%H%M"),".Rdata"))
```
...
for(i in 1:iters){
  ...
  out  = fMediation_ML(X,Y,t(M),mediatorMethod="fosr2s", nbasis=nbasis,norder=norder,lambda=lambda, plot=plots, boot=TRUE)
  if(cluster==TRUE){
    stat = boot(data = dta, statistic = fbootstrap_ML,R = nboot,sim = "ordinary",stype = "i",parallel = "multicore",ncpus=10)
    }else{
    stat = boot(data = dta, statistic = fbootstrap_ML,R = nboot,sim = "ordinary",stype = "i")
...
save(all,Pall,Pall2, file=paste0("Simulation_2_",format(Sys.time(), "%Y%m%d-%H%M"),".Rdata"))
```
...
save(all,Pall,Pall2, file=paste0("Simulation_3_",format(Sys.time(), "%Y%m%d-%H%M"),".Rdata"))
```