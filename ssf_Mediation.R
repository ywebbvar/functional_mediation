...
ssf_Mediation <- function(x,y,m,outcomeMethod="fosr2s", nbasis,norder,lambda=1e-8,pen=0.1, plot=FALSE, boot=FALSE){
  ...
  if(outcomeMethod=="fRegress"){
    ...
    if(plot) plotbeta(betaestlist = fRegressCell$betaestlist, betastderrlist = stderrCell$betastderrlist)
  }else{
  ...
  if(outcomeMethod=="fRegress"){
    ...
    ResM  = (m - eval.fd(tfine, fRegressCell$yhatfdobj$fd))
  ...
  if(mediatorMethod=="fRegress"){ 
    abf = a*eval.fd(tfine,bfun)      # ab-functions
  }else{
    abf = a*bf
  }
